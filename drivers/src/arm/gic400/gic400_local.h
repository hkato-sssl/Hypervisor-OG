...
errno_t gic400_dist_write_bit(const struct gic400 *gic, uint16_t bit_no, uintptr_t reg0);
...
static inline void gic400_write_dist(const struct gic400 *gic, uintptr_t reg, uint32_t d)
{
    REG_WRITE32(gic->config.base.dist, reg, d);
}

static inline void gic400_write_sync_dist(const struct gic400 *gic, uintptr_t reg, uint32_t d)
{
    REG_WRITE32(gic->config.base.dist, reg, d);
    memory_barrier();
...
static inline uint32_t gic400_read_dist(const struct gic400 *gic, uintptr_t reg)
{
    uint32_t d;
