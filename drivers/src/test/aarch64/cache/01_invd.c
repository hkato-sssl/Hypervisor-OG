 ...
 * 6) VA1に対してinvalidateを実行
 * 7) VA1/VA2の値が書き換え前のPAの値になる事を確認
 * 8) PAに値を書き込む
 * 9) VA1/VA2の値は変わらない事を確認
 * 10) VA1に対してinvalidateを実行
 * 11) VA1/VA2の値が書き換え前のPAの値になる事を確認
 *
 * 前提条件：
 * 1) MMU無効状態でテストコードを実行
...
errno_t test_aarch64_cache_01_invd(void)
{
    errno_t ret;
    volatile char *va1;
    volatile char *va2;
    ...
    ret = aarch64_mmu_map(&test_mmu, (void *)va1, test_region, sizeof(test_region), &attr);
    printk("VA1: aarch64_mmu_map() -> %d\n", ret);
    if (ret != SUCCESS) {
        return ret;
    }

    ret = aarch64_mmu_map(&test_mmu, (void *)va2, test_region, sizeof(test_region), &attr);
    ...
    printk("Invalidate VA1.\n");
    aarch64_dcache_invd((void *)va1);
    printk("VA1[0]: 0x%02x\n", va1[0]);
    printk("VA2[0]: 0x%02x\n", va2[0]);
    printk(" PA[0]: 0x%02x\n", test_region[0]);
    printk("\n");
