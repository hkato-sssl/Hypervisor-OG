 ...
 * エミュレーションされているかを確認する。
 * テスト内容は01.cと同じ。違いは仮想・物理間で異なる割り込み番号を
 * 設定している点にある。
 */
...
static struct soc_device ram = {
    ...
    .region.memory_type = SOC_MT_NORMAL_WB,
    .region.shareability = SOC_SH_ISH,
...
static struct soc_device uart0 = {
    ...
    .region.memory_type = SOC_MT_DEVICE_nGnRE,
    .region.shareability = SOC_SH_OSH,
    .region.access.flag.read = 1,
    .region.access.flag.write = 1,
...
static void init_devices(void)
{
    ...
    for (i = 0; i < NR_DEVICES; ++i) {
        ...
        dev->region.memory_type = SOC_MT_DEVICE_nGnRE;
        dev->region.shareability = SOC_SH_OSH;
...
void test_xilinx_mpsoc_02(void)
{
    printk("<%s>\n",__func__);
