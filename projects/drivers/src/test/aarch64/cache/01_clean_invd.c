/*
 * test/aarch64/cache/01_clean_invd.c
 *
 ...
 * 6) VA1に対してclean-invalidateを実行
 * 7) PAの値がVA1/VA2と同じ値になる事を確認
 * 8) PA/VA1の順にに異なる値を書き込む
 * 9) PAがVA1と同じ値である事を確認
 *
 * 前提条件：
 * 1) MMU無効状態でテストコードを実行
...
errno_t test_aarch64_cache_01_clean_invd(void)
{
    errno_t ret;
    volatile char *va1;
    volatile char *va2;
    ...
    printk("Clean & Invalidate VA1.\n");
    aarch64_dcache_clean_invd((void *)va1);
    printk("VA1[0]: 0x%02x\n", va1[0]);
    printk("VA2[0]: 0x%02x\n", va2[0]);
    printk(" PA[0]: 0x%02x\n", test_region[0]);
    printk("\n");
