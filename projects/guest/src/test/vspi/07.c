...
/*
 * テスト内容：マスク／アサート／アンマスク４
 * 実行手順
 *   IRQ無効化／PMR全マスク解除
 *   vSPI->SPI->SGIの順に可能な限り割り込みをアサート
 *   IRQ有効化
...
static void vspi_07_primary(void)
{
    printk("<%s>\n", __func__);
...
static void vspi_07_secondary(void)
{
    while (! start) {
        arm_wfe();
    }
