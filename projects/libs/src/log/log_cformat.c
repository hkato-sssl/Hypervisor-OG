...
static struct log_radix lower_case_hex = {
    16, lower_case_hex_chars, {2, lower_case_hex_prefix}};
static struct log_radix upper_case_hex = {
    16, upper_case_hex_chars, {2, upper_case_hex_prefix}};
...
int log_cformat(struct log_context *ctx)
{
    ...
    if ((ctx != NULL) && (ctx->request.ops != NULL)
        && (ctx->request.format != NULL)) {
        ret = cformat(ctx);
    } else {
        ret = -EINVAL;
    }
