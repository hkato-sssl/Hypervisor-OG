...
def out_func_prologue(func, reg):
    ...
    return T_FUNC_PROLOGUE.substitute(func=func, prefix=PREFIX, reg=reg)
...
def out_asm_header():
    ...
    t = string.Template(ASM_HEADER)
    header = t.substitute(file=OUT_FILE_NAME, owner=OWNER, year=datetime.date.today().year)
    return [header]
